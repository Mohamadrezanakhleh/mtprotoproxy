...
import runpy
import signal
from subprocess import check_output as sct
...
# Do Syncing time
DO_TIME_SYNC = config.get("DO_TIME_SYNC",False)
...
async def stats_printer():
    ...
    while True:
        ...
        try:
            if DO_TIME_SYNC:
                if ( "win" in pf):
                    sct(['w32tm', ' /resync'], shell=False, startupinfo=None).decode()

                elif ("lin" in pf ):
                    sct(['ntpd'], shell=False, startupinfo=None).decode()
                else:
                    pass
